    ...
    for (i = 0; i < length; i++) {
        ...
        if (p->isend == false) {
            for (int j = p->start; j < p->end; j++) {
                if (genome_array[p->ID][j] == contig[i]) {
                    i++;
                }
                else {
                    p->children[keepTrackOfNode] = createNode();
                    p->children[keepTrackOfNode]->ID = p->ID;
                    p->children[keepTrackOfNode]->start = p->start + j;
                    p->children[keepTrackOfNode]->isend = true;
                    p->isend = false;
                    p->end = j;
                    break;
                }
        ...
        if (!p->children[index]) {
            ...
            p->children[index]->start = startValue;
            if (p->isend == true) {
                int keepTrackOfNode = charToASCII(genome_array[p->ID][p->start + i]);
                for (int j = p->start; j < p->end; j++) {
                    if (genome_array[p->ID][j] == contig[i]) {
